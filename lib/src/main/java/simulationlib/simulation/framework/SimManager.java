...
public class SimManager<InputT, OutputT> {
    ...
    private final DashPlugInterface<InputT, OutputT> m_dashboardPlugin;
     ...
     */
    public SimManager(
            SimModelInterface<InputT, OutputT> simModelFunc,
            Client<Supplier<MultiType>> shuffleClient,
            DashPlugInterface<InputT, OutputT> dashboardPlugin,
        ...
        if (!RobotBase.isSimulation()) {
            throw new IllegalStateException(
                    "SimManager should only be instantiated when in simulation");
        }
    ...
    private void addPropertiesToGlobalHashMap(
        ...
        if (dashboardItems.length != multiTypes.length) {
            throw new IllegalArgumentException(
                    "dashboardItems and multiTypes must have the same length");
        }
    ...
    private DashboardItem[] getListOfDashboardPropertiesFromPlugin() {
        ...
        if (!checkIfDashboardItemsValid(result)) {
            System.out.println(
                    "WARNING: Dashboard plugin returned null or invalid list of properties");
            return null;
        }
    ...
    private void updateGlobalDashValues(MultiType[] newValues) {
        ...
        if (newValues.length != m_dashboardMultiTypeStorage.length) {
            System.out.println(
                    "WARNING: getDashboardPropertiesFromInputOutput() returned array of "
    ...
    private void doSimulationWrapper() {
        if (m_inputHandler != null && m_outputHandler != null) {
            ...
            if (m_pluginDashItemsInitialized) {
                MultiType[] newDashValues = m_dashboardPlugin.getDashboardPropertiesFromInputOutput(
                        input,
                        output);

                if (newDashValues == null) {
                    throw new IllegalStateException(
                            "getDashboardPropertiesFromInputOutput() returned null");
                }
