...
public class RamenArmSimLogic implements ExtendArmInterface {
     ...
     */
    public Pair<Boolean, Double> checkIfArmBroken(
     ...
     */
    public Pair<Boolean, Double> checkIfArmStuck(
            double oldSignedDegrees,
            boolean isOldSignedDegreesSet,
            double newSignedDegrees) {
        ...
        if (isOldSignedDegreesSet && isGrabberOpen && isInGrabberBreakRange(newSignedDegrees)
            ...
            // If the arm is ABOUT to go into the breakable range with the grabber open, the arm
            // gets
    ...
    private boolean isInGrabberBreakRange(double positionSignedDegrees) {
        return UnitConversions.lessThanButNotEqualDouble(
                positionSignedDegrees,
