package frc.robot.commands;

import edu.wpi.first.math.MathUtil;
import edu.wpi.first.math.controller.PIDController;
import edu.wpi.first.wpilibj2.command.Command;
import frc.robot.subsystems.SwerveDriveSystem;

public class RotateCommand extends Command {
    private SwerveDriveSystem m_swerveDrive;
    private double m_targetAngle;
    private PIDController m_pidController;

    public RotateCommand(SwerveDriveSystem swerveDrive, double targetAngle) {
        m_swerveDrive = swerveDrive;
        m_targetAngle = targetAngle;
        m_pidController = new PIDController(0.2, 0, 0);
        addRequirements(m_swerveDrive);

        m_pidController.setTolerance(0.1, 0.5);
        m_pidController.enableContinuousInput(0, Math.PI * 2);
    }

    @Override
    public void initialize() {
        m_swerveDrive.resetGyroToAngle(0);
    }

    @Override
    public void execute() {
        double speed = m_pidController
                .calculate(m_swerveDrive.getRotation2d().getRadians(), m_targetAngle);
        speed = MathUtil.clamp(speed, -0.01, 0.01);
        m_swerveDrive.drive(
                0,
                0,
                0.01);
    }

    @Override
    public boolean isFinished() {
        if (MathUtil
                .applyDeadband(
                        Math.abs(
                                Math.abs(m_swerveDrive.getRotation2d().getRadians())
                                        - Math.abs(m_targetAngle)),
                        0.1) == 0) {
            return true;
        }
        return false;
    }

    @Override
    public void end(boolean interrupted) {
        m_swerveDrive.stopSystem();
    }
}
